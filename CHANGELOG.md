...
- Continuous Windows/macOS/Linux native no-bundle smoke compilation for relevant source and dependency changes.
- Committed npm and Cargo lockfiles generated by their real package managers and verified before being adopted as reproducible build inputs.
- Platform support and portability documentation separating source/native compile evidence from packaged-release evidence.
- CI enforcement for the declared Rust 1.77.2 minimum supported version against the committed Cargo graph.
- A dedicated high-severity npm dependency audit alongside the existing Rust dependency audit and CodeQL scanning.

### Changed

- Frontend CI, cross-platform smoke, contributor setup, and release automation now install the exact npm dependency graph with `npm ci`.
- Rust quality, security, platform-smoke, benchmark, and release guidance now consume the committed Cargo graph with `--locked` verification where applicable.
- GitHub checkout/setup-node workflow runtimes were refreshed while preserving the existing release/security boundaries.
- CodeQL analysis now uses the current v4 action major.
- ESLint tooling was refreshed within the supported 9.x line, typescript-eslint was updated to a compatible maintained release, and the npm package now declares ESM explicitly to remove Node module-type ambiguity.
- Rust 1.98 Clippy compatibility was restored without changing analyzer or report-export semantics.
- Release, testing, setup, development, troubleshooting, governance, issue/PR templates, and contribution documentation now reflect the current dependency/version/platform gates.
